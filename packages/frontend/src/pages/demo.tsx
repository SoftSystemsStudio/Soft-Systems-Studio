...
export default function DemoPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const reply = data.reply || data.message || "I received your message but could not parse the response.";

            setMessages(prev => [...prev, { role: 'assistant', content: reply }]);
        } catch (error) {
            setMessages(prev => [...prev, { role: 'system', content: `ERROR: CONNECTION LOST - ${error}` }]);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
                    ...
                    <header className="p-6 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent pointer-events-auto">
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <a href="/" className="text-xs font-mono text-[#00ff88] border border-[#00ff88]/50 px-3 py-1 rounded hover:bg-[#00ff88]/10 transition-colors">
                                ← RETURN TO BASE
                            </a>
                    ...
                    <div className="p-4 md:p-8 flex justify-center pointer-events-auto">
                        <div className="w-full max-w-2xl bg-black/60 backdrop-blur-md border border-white/10 rounded-lg overflow-hidden shadow-2xl shadow-purple-900/20 flex flex-col h-[400px]">
                            ...
                            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 font-mono text-sm">
                                {messages.map((m, i) => (
                                    <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div
                                            className={`max-w-[80%] p-2 rounded ${m.role === 'user'
                                        ...
                                        >
                                            {m.role === 'assistant' && <span className="mr-2 text-xs text-indigo-500">[ORCHESTRATOR]</span>}
                                            {m.content}
                                        </div>
                                    </div>
                                ))}
                            ...
                            {/* Input Area */}
                            <form onSubmit={handleSubmit} className="border-t border-white/10 p-2 bg-black/40 flex">
