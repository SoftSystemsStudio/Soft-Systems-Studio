        ...
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #fff; padding: 32px; border-radius: 12px;">
          ...
          ${
            PAYMENT_LINKS[data.serviceInterest]?.url !== 'REPLACE_WITH_STRIPE_LINK'
              ? `
          <div style="background: rgba(34, 197, 94, 0.1); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(34, 197, 94, 0.3);">
            <h2 style="color: #22c55e; font-size: 16px; margin: 0 0 12px 0;">💳 Payment Link (${PAYMENT_LINKS[data.serviceInterest].deposit} deposit)</h2>
            <a href="${PAYMENT_LINKS[data.serviceInterest].url}" style="color: #22c55e; word-break: break-all;">${PAYMENT_LINKS[data.serviceInterest].url}</a>
          </div>
          `
              : ''
          }
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #fff; padding: 32px; border-radius: 12px;">
          ...
          ${
            PAYMENT_LINKS[data.serviceInterest]?.url !== 'REPLACE_WITH_STRIPE_LINK'
              ? `
          <div style="background: rgba(168, 85, 247, 0.15); padding: 24px; border-radius: 12px; margin: 24px 0; border: 1px solid rgba(168, 85, 247, 0.4); text-align: center;">
            <p style="margin: 0 0 16px 0; color: #fff; font-size: 16px; font-weight: bold;">
              Ready to get started right away?
            </p>
            <p style="margin: 0 0 20px 0; color: #d1d5db; font-size: 14px;">
              Secure your spot with a ${PAYMENT_LINKS[data.serviceInterest].deposit} deposit and we'll begin work immediately.
            </p>
            <a href="${PAYMENT_LINKS[data.serviceInterest].url}" style="display: inline-block; background: #a855f7; color: #fff; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">
              Pay Deposit Now →
            </a>
